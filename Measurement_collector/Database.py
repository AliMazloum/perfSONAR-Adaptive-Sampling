...
class database:
    ...
    def write_measurement(self, message):
        ...
        # elif(message['per_flow_measurement']['type'] == "sample_report"):
        #     message['flow_start_time'] =  datetime.utcfromtimestamp(message['flow_start_time']).strftime("%Y-%m-%dT%H:%M:%S.%f%z")
        #     message['sample_report_time'] =  datetime.utcfromtimestamp(message['sample_report_time']).strftime("%Y-%m-%dT%H:%M:%S.%f%z")
            # print(message)
        else:
