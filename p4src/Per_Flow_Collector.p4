    ...
    control Ingress(
        ...
        inout ingress_intrinsic_metadata_for_tm_t        ig_tm_md)
        {
            ...
            // ***********************************************************************************************
                ...
                // Lpf<value_t, bit<1>>(size=1) lpf_rtt_1;
                // Lpf<value_t, bit<1>>(size=1) lpf_rtt_2;
                // value_t lpf_rtt_input;
                // value_t lpf_rtt_output_1;
                // value_t lpf_rtt_output_2;

            apply {
                hdr.tcp.in_port = ig_intr_md.ingress_port;
                if(hdr.ipv4.isValid() && hdr.tcp.isValid() && ig_intr_md.ingress_port != 140){ // apply CMS to get new long flows
                    ...
                    if(meta.pkt_type==PKT_TYPE_SEQ){
                        ...
                        if(meta.table_1_read==0){
                            ...
                            // lpf_rtt_input = (value_t)meta.rtt;
                            // lpf_rtt_output_1 = lpf_rtt_1.execute(lpf_rtt_input, meta.flow_id);
                            // lpf_rtt_output_2 = lpf_rtt_2.execute(lpf_rtt_output_1, 0);
                            // meta.rtt = lpf_rtt_output_2;
                            exec_update_rtt();
                        }
                    }
                        // lpf_rtt_input = (value_t)meta.rtt;
                        // lpf_rtt_output_1 = lpf_rtt_1.execute(lpf_rtt_input, 0);
                        // lpf_rtt_output_2 = lpf_rtt_2.execute(lpf_rtt_output_1, 0);
                        // meta.rtt = lpf_rtt_output_2;
                        // exec_update_rtt();
                }
